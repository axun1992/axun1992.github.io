...
2. 通过`AActor::NetUpdateFrequency`值，决定Actor是否需要更新。
    1. 如果不需要，跳过此Actor。
3. 如果`AActor::bOnlyRelevantToOwner`为true，通过在Actor的拥有连接查看器上调用`AActor::IsRelevancyOwnerFor`，检查与Actor拥有连接的相关性。（注：从源码上看无此操作，此操作应在**循环遍历排序后的Actor列表**步骤发生）
...
#### 循环遍历排序后的Actor列表
循环遍历当前连接的**考虑复制列表**（注：从源码分析此处应为**相关列表**）中的每个Actor:  
